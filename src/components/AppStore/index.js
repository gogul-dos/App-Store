...
class AppStore extends Component {
  state = {currentCategory: 'SOCIAL', inputSearch: ''}
  ...
  filterApp = () => {
    const {currentCategory, inputSearch} = this.state
    let filteredAppsList = appsList.filter(
      eachApp => eachApp.category === currentCategory,
    )
    if (inputSearch.trim() !== '') {
      filteredAppsList = filteredAppsList.filter(eachApp =>
  ...
  render() {
    ...
    return (
      <div className="main-container">
        <h1>App Store</h1>
        <div>
          <input onChange={this.changed} placeholder="Search" type="search" />
          <img
        ...
        <ul className="tab-list">
          {tabsList.map(eachTab => (
            <TabItem
              tab={eachTab}
              key={eachTab.tabId}
              isThis={eachTab.tabId === currentCategory}
